...
if [[ -n $(hyprctl -j clients | jq -r ".[] | select(.class==\"$CLASS\" and .workspace.id!=$ACTIVE_WORKSPACE)") ]]; then
  # save current window address so we can restore back to it
  hyprctl activewindow -j | jq -r '.address' > /tmp/hdrop_last_focused
  ...
  if [[ $FOCUS == false ]]; then
    # if there was a previously-focused window, go back to that focus
    ...
    if [[ $FLOATING ]]; then hyprctl dispatch -- resizewindowpixel exact "$WIDTH_xy% $HEIGHT_xy%", "class:^$CLASS$" || notify "hdrop: Error resizing window for active monitor"; fi
    if [ -f /tmp/hdrop_last_focused ]; then
      LAST_FOCUSED=$(cat /tmp/hdrop_last_focused)
      hyprctl dispatch focuswindow address:${LAST_FOCUSED}
      rm /tmp/hdrop_last_focused
    fi
